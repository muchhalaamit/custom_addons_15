...
class RegisterBooks(models.Model):
    ...
    non_return_quantity = fields.Integer(string="Remaining Quantity", compute="_compute_remaining_quantity")
    ...
    def onchange_book_type(self):
        for rec in self:
            book_type_search = self.env["book.details"].search([("id", "=", self.book_name_id.id)])

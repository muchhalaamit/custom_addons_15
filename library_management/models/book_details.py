...
class BookDetails(models.Model):
    ...
    book_author_id = fields.Many2one("book.author", string="Author")
    book_quantity = fields.Integer(string="Book Quantity")
    book_code = fields.Char(string="Book Code")
    available_books = fields.Integer(string="Available Book", compute="_compute_book_quantity")
    ...
    def create(self, vals):
        if not vals.get("book_id"):
            ...
            vals["book_id"] = (seq[0:4] + "/" + datetime.date.today().strftime("%b").upper() + "/" + seq[4:])
        return super(BookDetails, self).create(vals)

    def name_get(self):
        result = []
        for rec in self:
            name = (rec.book_code + " - " + rec.book_name + " - " + rec.book_author_id.author_name)
    ...
    def _compute_book_quantity(self):
        for rec in self:
            issued_quantity = self.env["register.date"].search_count([("issued_book_id", "=", rec.id), ("returned_date", "=", False)])
